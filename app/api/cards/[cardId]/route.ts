...
export const DELETE = auth(async function DELETE(request: NextAuthRequest, { params }: { params: { cardId: string; }; }) {
  try {
    if (!request.auth) {
...
export const PATCH = auth(async function PATCH(request: NextAuthRequest, { params }: { params: { cardId: string; }; }) {
  try {
    if (!request.auth) {
      return responses.notAuthenticated();
    }
    ...
    const { japanese, japaneseSynonyms, english, englishSynonyms, hint } = body;
    const { cardId } = params;
...
export const GET = auth(async function GET(request: NextAuthRequest, { params }: { params: { cardId: string; }; }) {
  try {
    if (!request.auth) {
      return responses.notAuthenticated();
    }
